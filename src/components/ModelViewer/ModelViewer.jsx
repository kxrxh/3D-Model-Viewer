...
const AssemblyStateManager = lazy(
	() => import("./controls/AssemblyStateManager"),
);
const PartFocusController = lazy(
	() => import("./controls/PartFocusController"),
);
...
function useAssemblyState() {
	...
	const handleAddState = useCallback(() => {
		setAssemblyStates((prev) => [
			...prev,
			{ name: `Сборка ${prev.length + 1}`, parts: [], description: "" },
		]);
	}, []);
...
const PerformanceProfileSelector = React.memo(
...
export default function ModelViewer() {
	...
	const computedVisibleParts = useMemo(
		() => getVisibleParts(),
		[getVisibleParts],
	);
			...
			<Canvas
				...
				onCreated={({ gl }) => {
					...
					gl.physicallyCorrectLights =
						profiles[activeProfile].physicallyCorrectLights;
				}}
			>
	...
	return (
		<div className="w-full h-screen relative bg-gradient-to-br from-slate-100 to-slate-200">
			{!modelUrl && <ModelUploader onFileUpload={handleFileUpload} />}
