...
async function convertToWav(inputPath) {
	return new Promise((resolve, reject) => {
		...
		console.log(
			`Converting audio to WAV format: ${inputPath} -> ${outputPath}`,
		);
		...
		ffmpegProcess.on("close", (code) => {
			if (code !== 0) {
				...
				console.log(
					`Successfully converted audio to WAV format: ${outputPath}`,
				);
				resolve(outputPath);
			}
		});
...
async function transcribeWithWhisper(audioPath, language) {
	return new Promise((resolve, reject) => {
		...
		const args = [
			...
			"-l",
			language || "ru", // Language setting
			"-otxt", // Output text format
			"-of",
			outputFileBase, // Output file base (whisper-cli will add .txt)
		...
		whisperProcess.on("close", async (code) => {
			...
			if (code !== 0) {
				...
				return reject(
					new Error(`Whisper process failed with exit code ${code}`),
				);
			}
			...
			try {
				...
				if (
					!existsSync(expectedOutputFile) &&
					existsSync(alternateOutputFile)
				) {
				...
				if (existsSync(outputFile)) {
					...
					console.error(
						"Output file not found. Checked paths:",
						expectedOutputFile,
						alternateOutputFile,
					);
...
async function startServer() {
	try {
			...
			async fetch(req) {
				...
				if (path.startsWith("/api/speech-to-text") && req.method === "POST") {
					try {
						...
						console.log(
							`Audio file details - name: ${audioFile.name || "unnamed"}, type: ${audioFile.type || "unknown"}, size: ${audioFile.size} bytes`,
						);
						...
						if (audioFile instanceof File || audioFile instanceof Blob) {
							...
							console.log(
								`Audio saved to ${filePath}, size: ${audioBuffer.byteLength} bytes`,
							);
						} else {
							return new Response(
				...
				try {
					...
					if (
						existsSync(filePath) &&
						(await Bun.file(filePath).stat()).isFile()
					) {
		...
		console.log(
			`📝 Speech-to-text API available at http://localhost:${port}/api/speech-to-text`,
		);
