...
const FileUploader: FC<FileUploaderProps> = ({
	...
	<div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-gradient-to-b from-gray-50 to-gray-100">
		<div className="max-w-3xl w-full">
			<h1 className="text-5xl font-bold mb-3 text-gray-800 tracking-tight">Плеер сборки</h1>
			<p className="text-xl text-gray-500 mb-10">Загружайте и просматривайте 3D модели с инструкциями</p>
			...
			<div className="backdrop-blur-sm bg-white/80 p-8 rounded-2xl shadow-xl border border-gray-100 transition-all duration-300 hover:shadow-2xl">
				...
				>
					...
					{/* Icon Animation */}
					<div className={`transition-all duration-500 transform ${!hasModel || !hasInstructions ? "group-hover:scale-110" : ""}`}>
						{hasModel && hasInstructions ? (
							<FiCheckCircle size={72} className="text-green-500" />
						) : (
							<FiUploadCloud size={72} className={`${hasModel || hasInstructions ? "text-red-600" : "text-red-700"}`} />
						)}
					</div>
				...
				<div className="flex flex-wrap gap-3 mt-6 justify-center">
					<span className="px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-medium border border-gray-200">.glb</span>
					<span className="px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-medium border border-gray-200">.gltf</span>
					<span className="px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-medium border border-gray-200">.json</span>
					<span className="px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-medium border border-gray-200">.zip</span>
				</div>
			</div>
			...
			{(hasModel || hasInstructions) && (
				<div className="mt-6 grid grid-cols-2 gap-5">
					<div className={`p-5 rounded-xl flex items-center ${hasModel ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}>
						<div className={`mr-4 ${hasModel ? "text-green-500" : "text-yellow-500"}`}>
							{hasModel ? <FiCheckCircle size={30} /> : <FiAlertCircle size={30} />}
						</div>
						<div className="flex flex-col items-start">
							<span className="font-semibold text-lg text-gray-700">3D Модель</span>
							<span className={`text-base ${hasModel ? "text-green-600" : "text-yellow-600"}`}>
								{hasModel ? "Загружена ✓" : "Не загружена"}
							</span>
						</div>
					</div>
					
					<div className={`p-5 rounded-xl flex items-center ${hasInstructions ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}>
						<div className={`mr-4 ${hasInstructions ? "text-green-500" : "text-yellow-500"}`}>
							{hasInstructions ? <FiCheckCircle size={30} /> : <FiAlertCircle size={30} />}
						</div>
						<div className="flex flex-col items-start">
							<span className="font-semibold text-lg text-gray-700">Инструкции</span>
							<span className={`text-base ${hasInstructions ? "text-green-600" : "text-yellow-600"}`}>
