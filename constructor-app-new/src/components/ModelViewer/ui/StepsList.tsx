import { IoLayersOutline, IoSaveOutline, IoTrashOutline, IoCreateOutline, IoAddOutline, IoRemoveOutline, IoChevronDownOutline, IoChevronUpOutline } from "react-icons/io5";
...
const StepsList: React.FC<StepsListProps> = ({
  ...
  const groupPartsByHierarchy = useCallback((parts: string[]): PartGroup => {
      ...
      for (let i = 0; i < segments.length - 1; i++) {
        ...
        if (!group) {
          group = {
            ...
            isGroup: true,
            parts: [],
            expanded: expandedGroups[segments.slice(0, i + 1).join("/")] ?? true,
          };
  ...
  const RenderPartGroup = useCallback(({
    ...
    const isExpanded = currentPath in expandedGroups ? expandedGroups[currentPath] : true;
      ...
      <div>
        {group.name !== "root" && (
          ...
          >
            ...
            <span className="font-medium text-xs truncate flex-1" title={group.name}>
              {group.name}
            </span>
          </button>
        )}
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center mb-3">
          <h4 className="text-sm font-medium text-gray-600 flex items-center">
            <IoLayersOutline className="mr-1.5" size={16} /> Созданные шаги: {steps.length}
          </h4>
        ...
        {steps.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-2 p-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        {step.parts.length} {step.parts.length === 1 ? "деталь" : "деталей"}
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="p-3 pt-0 border-t border-gray-200 mt-2 bg-white">
                      <div className="text-xs font-medium text-gray-700 mb-1.5">Детали шага:</div>
                      <div className="max-h-60 overflow-y-auto">
                        <RenderPartGroup group={groupPartsByHierarchy(step.parts)} />
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">
            Пока не создано ни одного шага. Создайте первый шаг, настроив видимость деталей и заполнив форму выше.
          </div>
        )}
      </div>
    </div>
